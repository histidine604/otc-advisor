...
// Utility: add option safely with fallback list if JS failed earlier
function populateAilmentSelect(selectEl, ailmentsObj) {
  selectEl.innerHTML = "";
  const entries = Object.entries(ailmentsObj);
  if (!entries.length) {
    ["Fever","Cough","Heartburn / Indigestion","Constipation","Allergic Rhinitis","Nasal Congestion","Sore Throat","Diarrhea","Common Cold"].forEach((name, idx) => {
      const opt = document.createElement("option");
      opt.value = name.toLowerCase().replace(/[^a-z]+/g,"_"); opt.textContent = name;
  ...
  entries.forEach(([key, obj]) => {
    const opt = document.createElement('option');
    opt.value = key; opt.textContent = obj.name; selectEl.appendChild(opt);
  });
}

// Data + rules (All previous + Common Cold)
const DATA = {
  ailments: {
    cough: {
      name: "Cough",
    ...
    sore_throat: {
      ...
      recommend: (a) => {
        ...
        if (sx.includes("Drooling/trouble swallowing")) refer.push("Drooling/trouble swallowing.");
        if (duration === ">7 days") refer.push("Sore throat >7 days.");
        if (sx.includes("High fever") && duration !== "<3 days") notes.push("Consider strep evaluation if fever + sore throat without cough.");

        nonDrug.push("Warm fluids, honey (≥1 year), saline gargles.");
        if (agegrp !== "0-1") nonDrug.push("Throat lozenges/sprays for comfort (age-appropriate).");

        if (!refer.length) {
          recs.push({
            title: "Analgesic",
            examples: ["Acetaminophen", "Ibuprofen (≥6 months)"],
            how: "Use as directed for pain/fever.",
            warn: "Avoid ibuprofen in dehydration/ulcer risk; avoid duplicate acetaminophen."
          });
        }
    ...
    diarrhea: {
      ...
      questions: [
        ...
        { id: "sx", type: "multiselect", label: "Any of these?", options: [
          ...
          "Severe abdominal pain",
          "High fever",
          "Signs of dehydration (dry mouth, low urine)",
          "Recent antibiotics",
          "Travel exposure"
        ]},
        { id: "preg", type: "select", label: "Pregnant?", options: ["No","Yes"]}
      ...
      recommend: (a) => {
        ...
        if (sx.includes("Blood or black stool")) refer.push("Bloody/black stool.");
        if (sx.includes("Severe abdominal pain")) refer.push("Severe abdominal pain.");
        if (duration === ">3 days") refer.push("Diarrhea >3 days.");
        if (sx.includes("Signs of dehydration (dry mouth, low urine)")) refer.push("Dehydration.");

        nonDrug.push("Oral rehydration solution first-line; small frequent sips.");
        nonDrug.push("Avoid high-sugar drinks; consider bland diet (BRAT as tolerated).");

        if (!refer.length) {
          if (agegrp === ">12") {
            if (preg === "Yes") {
              recs.push({
                title: "Oral rehydration; consider bismuth with caution",
                examples: ["Bismuth subsalicylate"],
                how: "Use per label; avoid if aspirin allergy.",
                warn: "Discuss with clinician during pregnancy; salicylate risk."
              });
            } else {
              recs.push({
                title: "Loperamide for non-bloody, afebrile diarrhea",
                examples: ["Imodium (loperamide)"],
                how: "Use per label until stool forms (short term).",
                warn: "Do not use with fever/bloody stools."
              });
              recs.push({
                title: "Bismuth subsalicylate",
                examples: ["Pepto-Bismol (bismuth subsalicylate)"],
                how: "Helps with diarrhea and nausea.",
                warn: "Avoid with anticoagulants, aspirin allergy; Reye’s risk in children/teens recovering from viral illness."
              });
            }
          } else {
            notes.push("Children: focus on ORS; OTC antidiarrheals often not recommended without clinician guidance.");
          }
        }
        return { refer, notes, recs, nonDrug, showDosing: false };
      }
    ...
    common_cold: {
      ...
      questions: [
        AgeGroupInput("agegrp", "Age group"),
        { id: "duration", type: "select", label: "How long have symptoms been present?", options: ["<1 week","1–2 weeks",">2 weeks"], required: true },
        { id: "symptoms", type: "multiselect", label: "Which symptoms do they have?", options: [
          "Runny nose",
          "Sneezing",
          "Nasal congestion",
          "Dry cough",
        ...
        { id: "redflags", type: "multiselect", label: "Any red flags?", options: [
          "Shortness of breath or chest pain",
          "Severe unilateral facial pain/swelling",
          "Persistent high fever (≥102°F) >2 days",
          "Severe ear pain (especially in child)",
      ...
      recommend: (a) => {
        ...
        const sx = a.symptoms || [];
        const conditions = a.conditions || [];
        const red = a.redflags || [];

        // Red flags
        if (red.length) refer.push("One or more red flags selected.");
        if (duration === ">2 weeks") refer.push("Symptoms >2 weeks.");

        // Non-drug baseline
        nonDrug.push("Rest, hydration, humidified air.");
        nonDrug.push("Saline nasal spray/irrigation.");
        if (agegrp !== "0-1") nonDrug.push("Honey for cough (≥1 year).");

        // Pediatrics guardrail
        if (agegrp === "0-1") {
          notes.push("Avoid OTC cough/cold combination products in young children. Focus on non-drug measures and seek pediatric advice if not improving.");
        }

        const risky = (label) => conditions.includes(label);
        const preg = conditions.includes("Pregnancy");

        // Symptom-targeted recs
        const has = (label) => sx.includes(label);

        // Congestion
        if (has("Nasal congestion")) {
          recs.push({
            title: "Topical decongestant (short-term)",
            examples: ["Oxymetazoline 0.05% (Afrin)"],
            how: "Limit to 2–3 days to avoid rebound.",
            warn: "Avoid in small children unless label allows and clinician advises."
          });
          if (!(risky("Uncontrolled hypertension") || risky("Heart disease") || risky("Thyroid disease") || risky("Diabetes") || risky("MAOI use (or within 14 days)") || agegrp === "0-1" || preg)) {
            recs.push({
              title: "Oral decongestant",
              examples: ["pseudoephedrine (BTC)", "phenylephrine"],
              how: "Use daytime; can cause jitteriness/insomnia.",
              warn: "Avoid late evening; monitor BP if hypertensive."
            });
          } else {
            notes.push("Avoid oral decongestants due to conditions/pregnancy/age; prefer saline and intranasal steroid.");
          }
          recs.push({
            title: "Intranasal corticosteroid (INCS)",
            examples: ["Flonase (fluticasone)", "Rhinocort (budesonide)", "Nasacort (triamcinolone)"],
            how: "Daily for several days to reduce inflammation.",
            warn: "Not immediate; technique matters."
          });
        }

        // Runny nose / sneezing
        if (has("Runny nose") || has("Sneezing")) {
          if (agegrp !== "0-1") {
            recs.push({
              title: "Antihistamine (daytime: non-sedating)",
              examples: ["cetirizine", "loratadine", "fexofenadine"],
              how: "Helps more with sneezing/itching/runny nose than congestion.",
              warn: "Cetirizine may cause mild drowsiness."
            });
            recs.push({
              title: "Antihistamine (night: sedating)",
              examples: ["diphenhydramine", "doxylamine"],
              how: "Consider at night for rhinorrhea/sneezing.",
              warn: "Next-day drowsiness, anticholinergic effects; avoid in elderly/BPH/glaucoma."
            });
          } else {
            notes.push("Avoid sedating antihistamines in infants; use saline and humidifier.");
          }
        }

        // Cough
        if (has("Dry cough") && agegrp !== "0-1") {
          recs.push({
            title: "Antitussive (Dextromethorphan)",
            examples: ["Delsym (dextromethorphan ER)"],
            how: "Use for bothersome dry cough.",
            warn: "Avoid with MAOIs; may cause drowsiness/dizziness."
          });
        }
        if (has("Wet/productive cough") && agegrp !== "0-1") {
          recs.push({
            title: "Expectorant (Guaifenesin)",
            examples: ["Mucinex (guaifenesin)"],
            how: "Hydration enhances effect.",
            warn: "Stop and seek care if worsening or >7 days."
          });
        }
        ...
        if (has("Sore throat")) {
          if (agegrp !== "0-1") {
            recs.push({
              title: "Lozenges/sprays",
              examples: ["Benzocaine/menthol lozenges", "Phenol spray"],
              how: "Use as directed for temporary relief.",
              warn: "Risk of numbness; avoid eating right after use."
            });
          }
          recs.push({
            title: "Analgesic",
            examples: ["Acetaminophen", "Ibuprofen (≥6 months)"],
            how: "Use as directed for pain/fever.",
            warn: "Avoid ibuprofen if dehydrated/ulcer risk; avoid duplicate APAP."
          });
        }

        // Fever or body aches/headache
        let showDosing = false;
        if (has("Fever") || has("Body aches") || has("Headache")) {
          showDosing = true;
          recs.push({
            title: "Analgesic/antipyretic",
            examples: ["Acetaminophen", "Ibuprofen (≥6 months)"],
            how: "Use as directed; consider dosing calculator.",
            warn: "Check combination cold/flu products for duplicate acetaminophen."
          });
        }

        // Time course note
        notes.push("Typical cold lasts 7–10 days; seek care if symptoms worsen or new high fever develops.");

        return { refer, notes, recs, nonDrug, showDosing };
      }
    }
  }
};
...
function init() {
  populateAilmentSelect($ailment, DATA.ailments);
